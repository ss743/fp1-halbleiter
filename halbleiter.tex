...
Wir haben für unterschiedliche Entfernungen die Ladungsträgerwolken mit dem Oszilloskop aufgenommen und an die einzelnen Datensätze eine Gaußkurve gefittet (siehe Abbildung \ref{wolken1}). Man erkennt hier bereits die exponentielle Abnahme der Amplitude mit der Zeit und die lineare Geschwindigkeit. Aus den Fitparametern $A$ und $\sigma$ sowie aus der Geschwindigkeit der Ladungsträgerwolke lassen sich nun die Lebenszeit, die Diffusionskonstante und die Beweglichkeit der freien Elektronen bestimmen. Die erwartete Verteilung der Elektronen ist:
\begin{align*}
...
\end{align*}
Dabei ist $x_c$ die Entfernung zwischen Laserstrahl und Messspitze, $A(t)=C\cdot\exp\left(-\frac t{\tau_e}\right)$ ein Fitparameter und $\sigma(t)$ die Breite der gefitteten Gaußfunktion.\\
...
Eine weitere Möglichkeit, die Beweglichkeit der Elektronen zu bestimmen, ist die Aufnahme von Ladungsträgerwolken bei fester Entfernung und verschiedenen Spannungswerten. 
