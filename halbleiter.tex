...
In Abbildung \ref{Aufbau2} ist der Versuchsaufbau zum Haynes-Shockley-Versuch zu sehen. Licht eines gepulstes Lasers wird hier durch ein Glasfaserkabel auf eine Germaniumprobe ($40\times 40\times 300\mathrm{mm}^3$, p-dotiert) geleitet.  Hierdurch entsteht eine Ladungsträgerwolke, welche durch die angelegte Spannung $U$ durch den Halbleiter wandert. Wir beobachten diese Wolke mit Hilfe eines Oszilloskops und einer Sonde.
...
Kernstück dieses Versuchsaufbaus ist der Halbleiterdetektor. Es stehen für diesen Versuchsteil zwei verschiedene Halbleiterdetektoren zur Verfügung:
...
\newpage
\section{Auswertung}
\subsection{Teil I: Berechnen der Bandlückenenergien}
...
\subsection{Teil II: Ladungsträgerwolke in Germanium}
Im zweiten Teil des Versuchs haben wir Ladungsträgerwolken innerhalb des Halbleiters mit dem Oszilloskop aufgenommen. Die erwartete Verteilung der Elektronen ist (siehe \ref{teil3theorie}):
\begin{align*}
c(t,x)&=C\cdot\exp\left(-\frac t{\tau_e}\right)\cdot\frac1{\sqrt{4\pi D_et}}\cdot\exp\left(-\frac{(x-\mu_eEt)^2}{4D_et}\right)\\
&=A(t)\cdot\frac1{\sqrt{2\pi\sigma^2(t)}}\cdot\exp\left(-\frac{(x-x_c(t))^2}{2\sigma^2(t)}\right)
\end{align*}
Aus den Fitparametern $A$ und $\sigma$ sowie aus der Geschwindigkeit der Ladungsträgerwolke lassen sich nun die Lebenszeit, die Diffusionskonstante und die Beweglichkeit der freien Elektronen bestimmen. 
Dabei ist $x_c$ die Entfernung zwischen Laserstrahl und Messspitze, $A(t)=C\cdot\exp\left(-\frac t{\tau_e}\right)$ die Amplitude und $\sigma(t)$ die Breite der gefitteten Gaußfunktion. Die Zeit $t$ nach der sich die Ladungsträgerwolke am Ort $x_c$ befindet, erhalten wir durch den Mittelwert $\mu$ der Gaußfunktion.

Die Berechnungen in diesem Auswertungsteil wurden mit dem Skript \code{wolke.R} (siehe \ref{wolke}) durchgeführt.

\subsubsection{Messungen bei fester Spannung}
\gra{teil3messreihe1}{Ladungsträgerwolken bei gleicher Spannung und unterschiedlichen Entfernungen\label{wolken1}}
Wir haben für unterschiedliche Entfernungen bei gleicher Spannung die Ladungsträgerwolken mit dem Oszilloskop aufgenommen und an die einzelnen Datensätze eine Gaußkurve gefittet (siehe Abbildung \ref{wolken1}). Man erkennt hier bereits die exponentielle Abnahme der Amplitude mit der Zeit und die lineare Geschwindigkeit. Die weiteren Parameter werden im Folgenden bestimmt.\\

\gra{teil3x_t}{Die Entfernung zwischen Laserstrahl und Messspitze über der Zeit\label{xuebert}}

\paragraph{Beweglichkeit der freien Elektronen} Die Beweglichkeit $\mu_e$ der freien Elektronen ergibt sich aus der Geschwindigkeit der Elektronen. Diese ergibt sich aus einem linearen Fit der Entfernung über der Zeit (siehe Abbildung \ref{xuebert}). Aus diesem Fit erhalten wir den Wert $v_e=\mu_eE=\mu_e\frac{U}{l}=(473\pm8)\,\frac{\text{m}}{\text{s}}$. Aus diesem ergibt sich mit der Spannung $U=(50\pm0.2)$ V und der Länge des Germaniumblocks $l=(29.99\pm0.01)$ mm für die Beweglichkeit der Elektronen der Wert $\mu_e=(2840\pm170)\,\frac{\text{cm}^2}{\text{Vs}}$.

\gra{teil3sigma_t}{Zeitliche Entwicklung der Breite $\sigma$\label{sigmat}}
\paragraph{Diffusionskonstante der freien Elektronen} Aus der zeitlichen Entwicklung des Fitparameters $\sigma$, der Breite der Gaußfunktion lässt sich die Diffusionskonstante $D_e$ bestimmen. Es gilt: $\sigma(t)=\sqrt{2D_et}$. Durch einen Fit dieser Funktion an die Daten (siehe Abbildung\ref{sigmat}) ergibt sich für die Diffusionskonstante nun der Wert $D_e=(110\pm3)\,\frac{\text{cm}^2}{\text{s}}$. 

\gra{teil3A_t}{Zeitliche Entwicklung des Fitparameters $A$\label{At}}
\paragraph{Lebenszeit der freien Elektronen} Aus der zeitlichen Entwicklung des Fitparameters $A$ (siehe Abbildung \ref{At}) bestimmen wir die Lebenszeit $\tau_e$ der freien Elektronen. Der Parameter verhält sich folgendermaßen: $A(t)=C\cdot\exp\left(-\frac t{\tau_e}\right)$. Aus dem Exponentialfit erhalten wir nun für die Lebenszeit den Wert $\tau_e=(5.41\pm0.03)\cdot10^{-6}s$.

\subsubsection{Messungen bei fester Entfernung}


\gra{teil3messreihe2}{Ladungsträgerwolken bei gleicher Entfernung und unterschiedlichen Spannungen\label{wolken2}}

In der zweiten Messreihe Wir haben für unterschiedliche Spannungen bei gleicher Entfernung die Ladungsträgerwolken mit dem Oszilloskop aufgenommen und an die einzelnen Datensätze eine Gaußkurve gefittet (siehe Abbildung \ref{wolken2}). Man erkennt hier wiederum die exponentielle Abnahme der Amplitude mit der Zeit und die lineare Geschwindigkeit. Aus den Daten bestimmen wir nun erneut die Beweglichkeit, die Diffusionskonstante und die Lebenszeit der freien Elektronen.\\

\gra{teil3U_t}{Kehrwert der Spannung über die Zeit\label{Ut}}
\paragraph{Beweglichkeit der freien Elektronen} 
Aus $v_e=\mu_eE=\mu_e\frac{U}{l}$ folgt $\frac1U=\frac{\mu_e}{xl}t$. Über den linearen Fit in Abbildung \ref{Ut} erhält man nun $\frac{\mu_e}{xl}=(4760\pm70)\,\frac1{\text{VS}}$ und daraus für die Beweglichkeit der Elektronen $\mu_e=(4200\pm300)\,\frac{\text{cm}^2}{\text{Vs}}$

\gra{teil3sigma_t2}{Zeitliche Entwicklung der Breite $\sigma$\label{sigmat2}}
\paragraph{Diffusionskonstante der freien Elektronen} Aus der zeitlichen Entwicklung des Fitparameters $\sigma$, der Breite der Gaußfunktion lässt sich wieder die Diffusionskonstante $D_e$ bestimmen. Es gilt: $\sigma(t)=\sqrt{2D_et}$. Durch einen Fit dieser Funktion an die Daten (siehe Abbildung\ref{sigmat2}) ergibt sich nun für die Diffusionskonstante nun der Wert $D_e=(19.2\pm1.8)\,\frac{\text{cm}^2}{\text{s}}$. 

\gra{teil3A_t2}{Zeitliche Entwicklung des Fitparameters $A$\label{At2}}
\paragraph{Lebenszeit der freien Elektronen} Wieder bestimmen wir aus der zeitlichen Entwicklung des Fitparameters $A$ (siehe Abbildung \ref{At2}) die Lebenszeit $\tau_e$ der freien Elektronen. Aus dem Exponentialfit an $A(t)=C\cdot\exp\left(-\frac t{\tau_e}\right)$ erhalten wir diesmal für die Lebenszeit den Wert $\tau_e=(2.8\pm0.5)\cdot10^{-6}s$.

\subsection{Berechnungen der Fehler}
Die Fehler auf die Ergebnisse haben wir über Gauß'sche Fehlerfortpflanzung aus den statistischen Fehlern auf die Fitparameter erhalten.
\begin{align*}
	s_y=\sqrt{\sum_{i=1}^N\left(\frac{\partial y}{\partial x_i}\cdot s_{x_i}\right)^2}
\end{align*}
\newpage
...
\graTwo[0.49]{Am_CdTE_Teil3}{Co_CdTe_Teil3}{CdTe-Detektor: a) $59.5$ keV Peak von $^{241}$Am und b) $122$ keV bzw. $136.5$ keV von $^{57}$Co\label{cdte}}
\graTwo[0.49]{Am_Si_Teil3}{Co_Si_Teil3}{Si-Detektor: a) $59.5$ keV Peak von $^{241}$Am und b) $122$ keV bzw. $136.5$ keV von $^{57}$Co\label{si}}
...
$59.5 keV$ Peak		& 0.031			    & 0.007		\\ \hline
$122.06 keV$ Peak	& 0.5 			    & 0.3	\\ \hline
$136.47 keV$ Peak	& 4.5 		     	& 1.2
\end{tabular}\\}
...
$59.5 keV$ Peak		& 0.083		        & 0.003		\\ \hline
$122.06 keV$ Peak	& 0.0383		    & 0.0014	\\ \hline
$136.47 keV$ Peak	& 0.030	         	& 0.004
\end{tabular}\\}
...
$59.5 keV$ Peak		& 0.0091		    & 0.0014 	\\ \hline
$122.06 keV$ Peak	& 0.0257			& 0.0017	\\ \hline
$136.47 keV$ Peak	& 0.026		     	& 0.004
\end{tabular}\\}
...
& $\tau_e=(45\pm2)\cdot10^{-6}s$
 \end{tabular}}}
 \caption{Ergebnisse von Versuchsteil II}
\end{table}
\vskip 10 pt
...
$59.5 keV$ Peak		& 0.031			    & 0.007	 &	0.014\\ \hline
$122.06 keV$ Peak	& 0.5 			    & 0.3    & 0.0183	\\ \hline
$136.47 keV$ Peak	& 4.5 		     	& 1.2    & 0.02
\end{tabular}\\ }
 \caption{Absorptionsrate
  der Halbleiterdetektoren}
\end{table}
...
\subsection{Diskussion}\label{Diskussion}
 ...
 Die Ergebnisse der Bandlückenenergien stimmen sowohl für Silizium als auch für Germanium innerhalb des Fehlers mit dem Literaturwert überein. Auffallend war, dass die aufgenommenen Spektren wider Erwarten nicht symmetrisch sind. Als Ursache vermuten wir hier eine Asymmetrie des Gitters, die durch Verkratzen und andere Beschädigungen entstanden sein kann. Da dies allerdings keine Auswirkungen auf den Schnittpunkt der im Rahmen der Auswertung gefitteten Geraden hat, war dies für die Auswertung nicht relevant. Bei der Auswertung haben wir außerdem die Fehler auf den x-Wert, die Energie der Photonen, die sich aus dem Winkel ergeben, vernachlässigt. Der Hauptgrund hierfür ist, dass diese gegenüber den y-Fehlern vernachlässigbar sind. Zudem wären diese nur schwer abschätzbar.\\
 
 Bei der Messung der Parameter der freien Elektronen erzielten wir zum großen Teil sehr stark vom Literaturwert abweichende Ergebnisse. Lediglich die Beweglichkeit der freien Elektronen stimmte in der zweiten Messreihe innerhalb einer Standardabweichung mit dem Literaturwert überein, die Diffusionskonstante der ersten Messreihe immerhin innerhalb von drei Standardabweichungen. Alle weiteren Werte sind ziemlich weit vom Literaturwert entfernt. Besonders die Ergebnisse für die Lebenszeit der freien Elektronen weichen sehr stark vom Literaturwert ab. Dies liegt unter anderem daran, dass die Ladungsträger durch den Laserpuls sehr nah an der Oberfläche (durchschnittlich $0.5\mu m$) erzeugt werden. Hier treten starke Gitterdefekte auf, welche die Lebenszeit deutlich verkürzen.
 
 Außerdem haben wir beobachtet, dass sich offenbar für die Messung der Diffusionskonstante die erste Messreihe und für die Messung der Beweglichkeit die zweite Messreihe deutlich besser eignet. Das bessere Ergebnis für die Beweglichkeit in der zweiten Messreihe könnte sein, dass das elektrische Feld keine perfekte Homogenität aufweist. Da wir in der zweiten Messreihe einen Fit über den Kehrwert der Spannung machen, spielen mögliche Inhomogenitäten möglicherweise eine kleinere Rolle.
 
 Bei der zweiten Messreihe (siehe Abbildung \ref{wolken2}) sehen wir, dass der Verlauf der Amplituden eine sichtbare Abweichung vom erwarteten Exponentialverlauf darstellt. Dies erklärt zumindest teilweise die größere Abweichung bei der Berechnung der Diffusionskonstante, da diese sich aus dem Exponentialfit berechnet.\\
 
 Beim Messen der Absorptionsrate in Teil III stimmt keiner der berechneten Werte im Rahmen des Fehlers mit dem Literaturwert überein. Der Wert für den Americium-Peak bei 59.5 keV stimmt immerhin noch innerhalb drei Standarabweichungen mit dem Literaturwert überein. Die schlechten Übereinstimmungen könnten an einer Inhomogenität der Proben liegen. Wenn die Proben nicht exakt an der gleichen Stelle über dem Detektor liegen erhalten wir nun unterschiedliche Emissionsraten, weshalb wir die Absorption der einzelnen Detektoren nicht miteinander vergleichen können. Außerdem traten bei der Messung des Cobalt-Spektrums wenig Counts auf, was den Fit erschwert und daher Ungenauigkeiten verursachen kann. Des Weiteren ist es möglich, dass die Angaben zu den Detektorflächen \cite{staat} falsch sind, was die Berechnung weiter verfälschen kann. 
 
 Zu den Energieauflösungen gab es keinen Literaturwert, mit dem wir die Werte vergleichen konnten. Die Werte für die Cobalt-Peaks sind miteinander vertretbar, weichen allerdings deutlich von den Werten für den Americium-Peak abweichen. Die Abweichungen in diesem Versuchsteil können auch in der recht kurzen Messdauer begründet liegen, da sich die Spektren nicht sehr deutlich abzeichnen.
...
\lstinputlisting[language=R]{r_files/expfit.R}
\subsubsection{sqrtfit.R}\label{srcsqrtfit}
\lstinputlisting[language=R]{r_files/sqrtfit.R}
\subsubsection{wolke.R}\label{wolke}
\lstinputlisting[language=R]{r_files/wolke.R}
\subsubsection{AuswertungAmCdTe.R}\label{AuswertungAmCdTe}
\lstinputlisting[language=R]{Auswertung_Teil_3/Auswertung_Am_CdTe.R}
\subsubsection{AuswertungSi.R}\label{AuswertungSi}
\lstinputlisting[language=R]{Auswertung_Teil_3/Auswertung_Si.R}
\subsubsection{LinFit.R}\label{LinFit}
\lstinputlisting[language=R]{Auswertung_Teil_3/Lin_Fit.R}
